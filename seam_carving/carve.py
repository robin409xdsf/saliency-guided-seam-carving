...
def _get_backward_seam(energy: np.ndarray) -> np.ndarray:
    ...
    h, w = energy.shape
    # Float64 accumulation retains gradient differences beside the 1e11 barrier.
    inf = np.array([np.inf], dtype=np.float64)
    cost = np.concatenate((inf, energy[0].astype(np.float64), inf))
    ...
    for r in range(1, h):
        ...
        for j, i in enumerate(min_idx):
            cost[j + 1] = energy[r, j] + choices[i - base_idx[j], j]
...
def resize(
    ...
    :param energy_mode: Policy to compute energy for the source image. Could be
        one of ``backward``, ``forward`` or ``saliency``. If ``backward``, compute the energy
    ...
    ``saliency`` uses gradient + saliency_weight * (S > saliency_threshold).
    S is a normalized spectral-residual map computed once, then carried along
    with removed/inserted seams. An optional finite [0, 1] saliency_map can be
    supplied. Threshold defaults to 0.05; weight defaults to the PPT's 1e11.
    Saliency mode cannot be combined with drop_mask (conflicting objectives).
    """
    src = _check_src(src)
    ...
    if energy_mode == EnergyMode.SALIENCY:
        from seam_carving.saliency import spectral_residual, saliency_mask

        if drop_mask is not None:
            raise ValueError("saliency mode cannot be combined with drop_mask")
        if (
            not np.isfinite(saliency_weight)
            or not 0 <= saliency_weight <= np.finfo(np.float32).max
        ):
            raise ValueError(
                "saliency_weight must be finite, nonnegative and fit float32"
            )
        sal = spectral_residual(src) if saliency_map is None else saliency_map
        mask = saliency_mask(sal, saliency_threshold)
        if mask.shape != src.shape[:2]:
            raise ValueError("saliency_map shape must match the image")
        aux_energy = mask.astype(np.float32) * np.float32(saliency_weight)
        energy_mode = EnergyMode.BACKWARD
    elif saliency_map is not None:
        raise ValueError("saliency_map requires energy_mode='saliency'")
